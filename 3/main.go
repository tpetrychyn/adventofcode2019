...
func main() {
	...
	for _, v := range moves {
		...
		if err != nil {
			panic(fmt.Sprintf("failed to parse move %s", v))
		}
		switch string(v[0]) {
		case "R":
	...
	for _, v := range moves {
		...
		case "R":
			for i:=0;i<magnitude;i++ {
				if grid[x][y] > 0 {
					log.Printf("crossover at %d %d %d", x, y, movesTwo + grid[x][y])
					if movesTwo + grid[x][y] < minDist {
						minDist = movesTwo + grid[x][y]
					}
				}
				movesTwo++
				x++
		...
		case "L":
			for i:=0;i<magnitude;i++ {
				if grid[x][y] > 0 {
					log.Printf("crossover at %d %d", x, y)
					if movesTwo + grid[x][y] < minDist {
						minDist = movesTwo + grid[x][y]
					}
				}
